...
typedef struct INode
{
    int Type;
    int Owner;
    uint8_t Permission_owner;//0:no access;1:read;2:write;3:read&write
    uint8_t Permission_other;
    int Size;
    int LinksCount;
    int DirectBlock[8];
    int IndirectBlock;
...
int Inode_permission_check(INode *file,int mode)
{
    if (user->uid==0) return 0;
  //  printf("file->Owner:%d\n",file->Permission_owner);
    if (file->Permission_owner<mode) return -1;
    if (file->Owner!=user->uid && file->Permission_other<mode) return -2;
    return 0;
}
...
int Inode_init(INode *inode,int type,int size,int linkscount,int parent)
{
    ...
    inode->Permission_owner=3;
    inode->Permission_other=1;
    inode->Size=size;
...
{   
    ...
    if (Inode_permission_check(dir,2)<0) return -2;
    file=strtok(file," \r\n");
            ...
            for (int j = 0; j < 8; j++)
            {
                if (dir_item[j].Available == 0 && strcmp(dir_item[j].Name, file) == 0 && dir_item[j].Type == 0)
                {
                   // printf("remove file:%s\n",file);
                    INode *node=&InodeTable[dir_item[j].Inode]; 
                    if (Inode_permission_check(node,2)<0) return -2;
        ...
        if (dir->DirectBlock[i] != 0)
        {
            ...
            for (int j = 0; j < 8; j++)
            {
                if (dir_item[j].Available == 0 && strcmp(dir_item[j].Name, subdir) == 0 && dir_item[j].Type == 1)
                {
                    ...
                    if (Inode_permission_check(subdir_inode,2)<0) return -2;
...
{
    INode* dir = &InodeTable[Dir];
    if (Inode_permission_check(dir,1)<0) return -2;
...
int Inode_write_file(int inode, int pos, char* buf, int len)
{
    ...
    if (Inode_permission_check(file,2)<0) return -2;
...
{
    INode* file = &InodeTable[inode];
    if (Inode_permission_check(file,1)<0) return -2;
